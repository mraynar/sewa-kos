    ...
    <div class="min-h-screen flex flex-col justify-center">
        <div class="flex min-h-screen bg-slate-100">
            <div class="hidden lg:flex lg:w-3/5 bg-primary relative overflow-hidden items-center justify-center">
                ...
                <img src="{{ asset('images/cover_web/kos3.jpg') }}" class="absolute inset-0 w-full h-full object-cover object-center scale-105 hover:scale-100 transition-transform duration-700" alt="Interior Kos">
            ...
            <div class="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 bg-gray-50/50">
                <div class="max-w-md w-full bg-white p-8 md:p-10 rounded-4xl border border-gray-300 shadow-2xl shadow-slate-200/80">
                    <div class="mb-8 text-center">
                        ...
                        <p class="text-sm text-slate-500 font-medium">Belum punya akun? <a href="{{ route('register') }}" class="text-primary font-bold hover:underline ms-0.5">Daftar Sekarang</a></p>
                    </div>
                    ...
                    <form action="{{ route('login') }}" method="POST" class="space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="text" name="email" required value="{{ old('email') }}" class="w-full px-4 py-3.5 bg-white border focus:border-primary rounded-xl transition-all font-medium text-slate-700 outline-none text-sm placeholder:text-gray-400 {{ $errors->has('email') ? 'border-red-400' : 'border-gray-200' }}" placeholder="example@gmail.com">
                            @error('email')
                        ...
                        <div class="space-y-1.5">
                            <div class="flex justify-between items-center px-1">
                                ...
                                <a href="{{ route('password.request') }}" class="text-xs font-bold text-slate-500 hover:underline">Lupa kata sandi?</a>
                            </div>
                            <div class="relative" x-data="{ show: false }">
                                <input :type="show ? 'text' : 'password'" name="password" required class="w-full pl-4 pr-12 py-3.5 bg-white border focus:border-primary rounded-xl transition-all font-medium text-slate-700 outline-none text-sm {{ $errors->has('password') ? 'border-red-400' : 'border-gray-200' }}" placeholder="Masukkan kata sandi">
                        ...
                        <div class="pt-6">
                            ...
                            <div class="grid grid-cols-3 gap-4">
                                <a href="{{ route('auth.google') }}" class="flex items-center justify-center py-3 border border-gray-300/70 rounded-xl hover:bg-gray-50 hover:border-primary/70 transition-all group">
